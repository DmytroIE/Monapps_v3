...
def get_end_rts(
    datafeeds: Iterable[Datafeed], t_resample: int, start_rts: int, num_df_to_process: int
) -> tuple[int, bool]:
    ...
    end_rts_by_max_num_df_readings = start_rts + t_resample * max_num_dfreadings_per_one_df_to_process
...
def get_df_value_map(datafeeds, start_rts: int, end_rts: int) -> DfValueMap:
